...
export const CatalogView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'materials' | 'prepTypes'>('materials');
  const [clinicalSpecs, setClinicalSpecs] = useState(getClinicalSpecs());
  const [showPrepModal, setShowPrepModal] = useState(false);
  const [editingPrep, setEditingPrep] = useState<ClinicalPrepType | null>(null);
  ...
  const persistPrepTypes = (next: ClinicalPrepType[]) => {
    const updated = { ...clinicalSpecs, prepTypes: next };
    setClinicalSpecs(updated);
    saveClinicalSpecs(updated);
  };
  ...
  const handlePrepSubmit = (e: React.FormEvent) => {
    ...
    if (!prepName.trim()) errs.name = 'Restoration type name is required';
    setPrepErrors(errs);
    if (Object.keys(errs).length > 0) return;

    if (editingPrep) {
      persistPrepTypes(clinicalSpecs.prepTypes.map((p) =>
        p.id === editingPrep.id
          ? { ...p, name: prepName.trim(), code: prepCode.trim(), description: prepDescription.trim(), is_active: prepActive }
          : p
      ));
    } else {
      const id = prepName.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '') || `prep_${Date.now()}`;
      persistPrepTypes([
        ...clinicalSpecs.prepTypes,
        { id, name: prepName.trim(), code: prepCode.trim() || undefined, description: prepDescription.trim() || undefined, is_active: prepActive }
      ]);
    }
    resetPrepForm();
  };

  const startPrepEdit = (p: ClinicalPrepType) => {
    setEditingPrep(p);
    setPrepName(p.name);
    setPrepCode(p.code || '');
    setPrepDescription(p.description || '');
    setPrepActive(p.is_active);
    setShowPrepModal(true);
  };

  const togglePrepActive = (p: ClinicalPrepType) => {
    persistPrepTypes(clinicalSpecs.prepTypes.map((x) => (x.id === p.id ? { ...x, is_active: !x.is_active } : x)));
  };

  const deletePrep = (p: ClinicalPrepType) => {
    if (confirm(`Remove "${p.name}" from restoration types? It will no longer appear in the FDI chart prep options.`)) {
      persistPrepTypes(clinicalSpecs.prepTypes.filter((x) => x.id !== p.id));
    }
  };
  ...
  return (
    <div className="space-y-6 print-page">
      ...
      {/* Module Tabs */}
      <div className="bg-white p-1.5 rounded-xl border border-slate-200 shadow-2xs flex items-center gap-1 w-max">
        <button
          onClick={() => setActiveTab('materials')}
          className={`px-4 py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
            activeTab === 'materials' ? 'bg-slate-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
          }`}
        >
          Materials & Pricing
        </button>
        <button
          onClick={() => setActiveTab('prepTypes')}
          className={`px-4 py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
            activeTab === 'prepTypes' ? 'bg-slate-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
          }`}
        >
          Restoration Types
        </button>
      ...
      {activeTab === 'prepTypes' && (
        <div className="space-y-4 print-flow">
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-sm font-bold text-slate-900">Restoration / Preparation Types</h2>
              <p className="text-xs text-slate-500 mt-0.5">
                These options appear in the FDI chart's "Restoration / Preparation Type" picker when charting case teeth.
              </p>
            </div>
            <button
              onClick={() => { resetPrepForm(); setShowPrepModal(true); }}
              className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold transition-colors shadow-2xs flex items-center gap-1.5 cursor-pointer shrink-0 no-print"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>Add Restoration Type</span>
            </button>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider text-slate-500 hidden md:table-cell">Description</th>
                  <th className="px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider text-slate-500">Status</th>
                  <th className="px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider text-slate-500 text-right no-print">Actions</th>
                </tr>
              </thead>
              <tbody>
                {[...clinicalSpecs.prepTypes].sort((a, b) => Number(b.is_active) - Number(a.is_active)).map((p) => (
                  <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-900">{p.name}</span>
                        {p.code && (
                          <span className="px-1.5 py-0.5 rounded font-mono text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200">{p.code}</span>
                        )}
                      </div>
                      <p className="md:hidden text-[11px] text-slate-500 mt-0.5 line-clamp-1">{p.description || '—'}</p>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <p className="text-xs text-slate-500 line-clamp-2 max-w-md">{p.description || '—'}</p>
                    </td>
                    <td className="px-4 py-3">
                      <button
                        onClick={() => togglePrepActive(p)}
                        title={p.is_active ? 'Active — click to hide from FDI chart' : 'Inactive — click to activate'}
                ...
                {clinicalSpecs.prepTypes.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-4 py-10 text-center text-xs text-slate-400">
                      No restoration types yet — add one to make it available on the FDI chart.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {/* Add / Edit Restoration Type Modal */}
      {showPrepModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-5 border border-slate-200 shadow-xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-sm">
                    {editingPrep ? 'Edit Restoration Type' : 'Add Restoration Type'}
                  </h3>
                  <p className="text-xs text-slate-400">Available in the FDI chart prep picker</p>
                </div>
              </div>
              <button onClick={resetPrepForm} className="text-slate-400 hover:text-slate-800 p-1 rounded-lg cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            </div>

            <form onSubmit={handlePrepSubmit} className="space-y-3.5">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Full Contour Crown"
                  className="w-full p-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:outline-none focus:border-slate-400 text-slate-900"
                />
                {prepErrors.name && <p className="text-[10px] text-rose-500 mt-1">{prepErrors.name}</p>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. FC"
                    maxLength={6}
                    className="w-full p-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:outline-none focus:border-slate-400 text-slate-900 uppercase"
                  />
                </div>
                <div>
                  ...
                  <button
                    ...
                    className={`w-full p-2 text-xs font-bold rounded-lg border transition-colors cursor-pointer ${
                      prepActive
                        ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                        : 'bg-slate-50 text-slate-500 border-slate-200'
                    }`}
                  >
                    {prepActive ? '● Active' : '○ Inactive'}
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Indications, margin design, lab notes..."
                  className="w-full p-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:outline-none focus:border-slate-400 text-slate-900 resize-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold shadow-2xs transition-colors cursor-pointer"
                >
                  {editingPrep ? 'Update Type' : 'Add Type'}
                </button>
              </div>
            </form>
          </div>
